...
export class ModalCreateEditComponent implements OnInit {
  ...
  initForm() {
    this.formGroupCreate = new FormGroup({
      ...
      stalls_id: new FormControl(null, [Validators.required]),
  ...
  selectStall() {
    let data = { category_id: this.formGroupCreate.value.category_id }
  ...
  save() {
    ...
    this.categoryGroupCreate = this.formGroupCreate.value;
    ...
    if (!this.itemDetail)//thêm mới
    {
      this.confirmDialog.confirm('Please confirm..', 'Bạn có thực sự muốn tạo mới bản ghi này ?')
        .then((confirmed) => {
          if (confirmed) {
            this.categoryService.Category().Group().add(this.categoryGroupCreate).subscribe((rs: any) => {
    ...
    else//sửa
    {
      this.confirmDialog.confirm('Please confirm..', 'Bạn có thực sự muốn chỉnh sửa bản ghi này ?')
        .then((confirmed) => {
          if (confirmed) {
            this.categoryGroupCreate.id = this.itemDetail.id;
            this.categoryService.Category().Group().edit(this.categoryGroupCreate).subscribe((rs: any) => {
              if (rs.statusCode == 200) {
                this.toast.success(rs.message);
              } else {
                this.toast.warning(rs.message);
