...
export class ModalCreateEditComponent implements OnInit {
  ...
  save() {
    ...
    this.categoryUnitsCreate = this.formGroupCreate.value;
    ...
    if (!this.itemDetail)//thêm mới
    {
      this.confirmDialog.confirm('Please confirm..', 'Bạn có thực sự muốn tạo mới bản ghi này ?')
        .then((confirmed) => {
          if (confirmed) {
            this.categoryService.Category().Unit().add(this.categoryUnitsCreate).subscribe((rs: any) => {
    ...
    else//sửa
    {
      this.confirmDialog.confirm('Please confirm..', 'Bạn có thực sự muốn chỉnh sửa bản ghi này ?')
        .then((confirmed) => {
          if (confirmed) {
            this.categoryUnitsCreate.id = this.itemDetail.id;
            this.categoryService.Category().Unit().edit(this.categoryUnitsCreate).subscribe((rs: any) => {
              if (rs.statusCode == 200) {
                this.toast.success(rs.message);
              } else {
                this.toast.warning(rs.message);
